#!/usr/bin/env python3
# five_seed_loso_rf_eval.py – evaluate fixed Random Forest hyper‑parameters across 5 seeds,
...
from ml_utils_svm import load_data_loso  # LOOCV loader function
...
def run_experiment(best_params, batch_size=18):
    ...
    pd.DataFrame(rows).to_csv('evaluation_runs_metrics_rf.csv', index=False)
    print("\n→ CSV saved: evaluation_runs_metrics_rf.csv\n")

if __name__ == '__main__':
    seed_everything(42)
    # replace with your best hyper‑parameterss
    best_params = {'n_estimators': 50, 'max_depth': 10, 'min_samples_split': 2, 'min_samples_leaf': 2, 'max_features': 'sqrt'}
