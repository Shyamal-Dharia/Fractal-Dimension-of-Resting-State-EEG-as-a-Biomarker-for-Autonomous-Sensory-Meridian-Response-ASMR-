...
import torch
import mne
from ml_utils import train, evaluate, load_data_loso
from models import spatial_mamba
...
def run_experiment():
    ...
    print("\n===== Starting experiment: spatial_mamba =====\n")
    ...
    for run_idx, seed in enumerate(run_seeds, start=1):
        ...
        # grid search (here only one combo, but structure kept)
        for combo in itertools.product(*hyper.values()):
            ...
            for fold_idx, fold in enumerate(folds, start=1):
                ...
                for epoch in range(1, p['num_epochs'] + 1):
                    ...
                    print(
                        f"[mamba] Run {run_idx} seed {seed} fold {fold_idx} "
                ...
                # subject‐level: threshold on fraction of predicted 1’s
    ...
    pd.DataFrame(rows).to_csv('evaluation_runs_metrics_spatial_mamba.csv', index=False)
    print("\n→ CSV saved: evaluation_runs_metrics_spatial_mamba.csv\n")
