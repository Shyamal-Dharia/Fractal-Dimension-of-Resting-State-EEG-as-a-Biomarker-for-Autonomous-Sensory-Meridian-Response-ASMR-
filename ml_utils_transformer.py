...
def train(model, train_dataloader, test_dataloader, optimizer, device,
    ...
    for inputs, labels in train_dataloader:
        ...
        outputs, source_feats, _ = model(inputs)
        target_out, target_feats, _ = model(test_inputs)
...
def evaluate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            outputs, mmd, attn_wei = model(inputs)
            ...
            all_labels.extend(labels.squeeze().cpu().numpy())
            # atten wei is a list
            attn_wei_list.extend([wei.cpu().numpy() for wei in attn_wei])

  
    avg_loss = epoch_loss / len(dataloader.dataset)
    acc = accuracy_score(all_labels, all_preds)
    f1 = f1_score(all_labels, all_preds, average='macro')
    return avg_loss, acc, f1, all_preds, all_labels, attn_wei_list
